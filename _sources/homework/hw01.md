...
#### (a)
Write down the discrete-time dynamics $\mathbf{x}_{k+1} = f(\mathbf{x}_k, \mathbf{u}_k)$ using Euler integration with step size $\Delta t$.
...
### 2. Gradient descent. 
Given the different functions studied in notebook 02, discuss how the choice of initial conditions and step size affects the convergence and the final solution when using (vanilla) gradient descent.
...
#### (a)
Demonstrate that the relative degree of $b$ with respect to the dynamically-extended unicycle dynamics is $2$ for both control inputs.
...
### 4. Closer loop at CBF-CLF-QP Filter demo.
