...
In the HJB equation, $V(x,t)$ is treated as a function of two independent variables,
...
Let’s check whether $\dot{P}+Q+2PA-PBR^{-1}B^TP$ symmetric by check it transpose:
...
For this equality to hold for all \(x\), the matrix inside the quadratic form must be zero:
...
For the infinite-horizon LQR problem, the final time $T$ goes to infinity. The value function no longer depends explicitly on time. Therefore, 
...
\dot{V}(x)
=
-(x^TQx + u^{\star T}Ru^\star)

$$

Since $Q\succeq 0$ and $R\succ 0$,
...
Therefore, $V(x)=x^TPx$ is a valid Lyapunov function, and the origin of the closed-loop system
...
Equivalently, this is the same gain obtained from the scaled system
...
K
=
(R+\tilde{B}^TP_\infty\tilde{B})^{-1}
\tilde{B}^TP_\infty\tilde{A}
...
2. Disturbances and uncertainty:
   ...
   The the nominal trajectory and LQR controller are designed based on the model without wind disturbance. Therefore, the optimal control sequence is optimal only for the nominal dynamics. so it cannot completely remove the offset.
...
To avoid this, we should modify the nominal trajectory optimization to include a larger obstacle clearance margin. For example, inflate the obstacle constraint:
